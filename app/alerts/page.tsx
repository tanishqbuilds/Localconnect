      ...
      <div className="space-y-8">
        {alerts?.map((alert) => (
          <div key={alert.alert_id} className={`group relative rounded-[2.5rem] p-10 overflow-hidden transition-all hover:shadow-2xl ${
            alert.level === 'Emergency' ? 'bg-red-600 text-white shadow-red-500/20' :
            alert.level === 'Warning' ? 'bg-amber-400 text-slate-900 shadow-amber-500/20' :
            'bg-white text-slate-900 border border-slate-200 shadow-sm'
          }`}>
            <div className="relative z-10 flex flex-col md:flex-row gap-8 items-start">
              <div className={`p-4 rounded-3xl shrink-0 ${
                alert.level === 'Emergency' ? 'bg-white/20' :
                alert.level === 'Warning' ? 'bg-slate-900/10' :
                'bg-primary-50 text-primary-600'
              }`}>
                {alert.level === 'Emergency' ? <ShieldAlert className="w-10 h-10" /> :
                 alert.level === 'Warning' ? <Zap className="w-10 h-10" /> :
              ...
              <div className="flex-1">
                <div className="flex items-center gap-4 mb-4">
                   ...
                   <span className={`text-[10px] font-bold uppercase tracking-widest flex items-center gap-1.5 ${
                     alert.level === 'Emergency' ? 'text-white/60' : 'text-slate-400'
                   }`}>
                ...
                <p className={`text-lg leading-relaxed mb-8 ${
                   alert.level === 'Emergency' ? 'text-white/80' : 'text-slate-500'
                }`}>
                  {alert.content}
                </p>
                ...
                <div className="flex items-center gap-6">
                   <div className={`flex items-center gap-2 text-xs font-black uppercase tracking-widest ${
                      alert.level === 'Emergency' ? 'text-white' : 'text-slate-900'
                   }`}>
                      <MapPin className="w-4 h-4 opacity-50" />
                      {alert.broadcast_city}
                   </div>
                   <div className={`h-1 flex-1 rounded-full ${
                      alert.level === 'Emergency' ? 'bg-white/20' : 'bg-slate-100'
                   }`} />
            ...
            <div className={`absolute bottom-0 right-0 p-12 transition-transform duration-500 group-hover:scale-150 ${
               alert.level === 'Emergency' ? 'text-white/5' : 'text-slate-50'
            }`}>
